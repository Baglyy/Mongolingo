...
async function initializeData() {
  try {
    ...
    // Sauvegarde l'état 'unlocked' des niveaux existants
    const existingLevels = await Level.find({});
    const unlockedStates = {};
    existingLevels.forEach((level) => {
      unlockedStates[level.id] = level.unlocked;
    });
    ...
    // Restaure l'état 'unlocked' des niveaux existants
    for (const [levelId, unlocked] of Object.entries(unlockedStates)) {
      await Level.updateOne({ id: Number(levelId) }, { $set: { unlocked } });
    }
